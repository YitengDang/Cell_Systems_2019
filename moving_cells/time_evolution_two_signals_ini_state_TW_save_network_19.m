...
sigma_D_all = [0.003 0.03]; %10.^[-3 -2 -1];
...
%(!!!)  % Filename pattern (!!!)
        pattern = strrep(sprintf('%s_N%d_ini_state_TW_params_%d_sigma_D_%.3f_t_out_%s_period_%s',...
...
for idx_loop=1:numel(sigma_D_all) %numel(mcsteps_all)
    ...
    for trial=1:sim_count
        ...
        positions = {};
        distances = {};
        fname_str_template = strrep(sprintf('%s_N%d_ini_state_TW_params_%d_sigma_D_%.3f',...
