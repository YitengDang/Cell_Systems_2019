...
for idx_1=1:numel(a0_list)
    for idx_2=1:numel(Con_list)
        for idx_3=1:numel(K_list)
            ...
            for idx_4=1:numel(p_ini_list)
                this_p_ini = p_ini_list(idx_4);
                
                % Filename pattern 
                %--------------------------------------------------------------
                pattern = strrep(sprintf('N%d_a0_%.2f_K_%d_Con_%d_hill_%.2f_p_ini_%.2f_t_out_%s-v%s',...
                        N, this_a0, this_K, this_Con, hill, this_p_ini,...
                        '(\d+)', '(\d+)'), '.', 'p');
                %--------------------------------------------------------------
                listing = dir(folder);
                ...
                sim_to_do(idx_1, idx_2, idx_3, idx_4) = num_sims-filecount;
            end
        end
    end
end
...
for idx_a0=1:numel(a0_list)
    ...
    for i=1:numel(Con_list)
        ...
        for j=1:numel(K_list)
            ...
            for idx_p = 1:numel(p_ini_list)
                p_ini = p_ini_list(idx_p);
                
                fprintf('a0 = %.2f, K=%d, Con=%d, p_ini = %.2f \n', a0, K, Con, p_ini);
                
                % default filename
                fname_str_template = strrep(sprintf('N%d_a0_%.2f_K_%d_Con_%d_hill_%.2f_p_ini_%.2f',...
                    N, a0, K, Con, hill, p_ini), '.', 'p');
                display_fig = 0;

                for tests = 1:sim_to_do(idx_a0, i, j, idx_p)
