% Computes the probability of crossing into another basin of attraction (n=2) by
% simulation
close all
clear variables
...
ini_state = 0; %start in low state (0) / high state (1)
cells_ini = fp(2*ini_state+1)*ones(N,1);
%%
% Plot
%{
h1=figure(1);
...
noise_all = 10^(2); %10.^(0.2:0.1:1); %(-1:0.1:1);
for trial=1:numel(noise_all)
    noise = noise_all(trial);
    x_out = zeros(nsim, tmax+1); % mean x
    x_out(:, 1) = mean(cells_ini);  
    %sim = 1;
    %x_state = zeros(nsim, 1);
    for sim=1:nsim
        fprintf('trial = %d, sim = %d \n', trial, sim);
        cells = cells_ini; %initial state
        for t=1:tmax
            cells = update_cells_noise_hill(cells, dist, Con, K, a0, Rcell, noise, hill, 10);
            x_out(sim, t+1) = mean(cells);
            %switch x_state(sim)
            %    case 0
    ...
    fname_str = strrep(sprintf('uniform_lattice_trajectories_N%d_Con%d_K%d_a0_%.2f_hill%.2f_noise_%.3f_tmax%d_nsim%d_ini_state%d', ...
        N, Con, K, a0, hill, noise, tmax, nsim, ini_state), '.', 'p');
    ...
    if qsave
        save(file_out, 'a0', 'Con', 'edges', 'fN', 'fp', 'gridsize', 'hill', 'K',...
            'N', 'noise', 'nsim', 'x_cross1', 'x_cross2', 'x_out_min', 'tmax');
    end
end
...
for i=1:numel(noise_all)
    noise = noise_all(i);
    fname_in = strrep(sprintf('uniform_lattice_trajectories_N%d_Con%d_K%d_a0_%.2f_hill%.2f_noise_%.3f_tmax%d_nsim%d_ini_state0', ...
    ...
    %{
    fname_in = strrep(sprintf('uniform_lattice_trajectories_N%d_Con%d_K%d_a0_%.2f_hill%.2f_noise_%.3f_tmax%d_nsim%d_ini_state1', ...
        N, Con, K, a0, hill, noise, tmax, nsim), '.', 'p');
    %if exist( fullfile(path, fname_in) ) == 4
        load(fullfile(path, fname_in));
        %frac_final(i) = sum(x_out_final > fp(2))/nsim;
