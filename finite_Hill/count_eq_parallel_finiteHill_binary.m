function [count, t_av, I_av] = count_eq_parallel_finiteHill_binary(...
...
% k runs from 0 to nvals, <Xi>(initial) = k/nvals
...
t_av = zeros(N+1,1);
I_av = zeros(N+1,3);
%edges = linspace(0,1,N+1); % values for <Xi>
...
for k = 0:N
	...
	if k == 0 %special case all cells OFF; only one state -> one simulation enough
        ...
        kout = round(sum(cells)); %round to nearest integer 
