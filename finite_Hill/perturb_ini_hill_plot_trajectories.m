...
%fpattern = strrep(sprintf('spin_flip_N%d_n%d_flip%s_a0_%.1f_K_%d_Con_%d_hill_%.2f-v%s', ...
%            N, iniON, straux, a0, K, Con, hill, straux), '.', 'p');
fpattern = strrep(sprintf('perturb_ini_N%d_pini%.3f_a0_%.1f_K_%d_Con_%d_hill_%.2f_pnoise_%.2f-v%s', ...
...
if qsave
    out_file = fullfile(pwd, 'figures', 'single_spin_flip',... 
        strcat(fname_out,'_hmap_pI')); %filename
...
if qsave
    out_file = fullfile(pwd, 'figures', 'single_spin_flip',... 
        strcat(fname_out,'_h_traj')); % filename
...
% save hamming dist trajectories
qsave = 0;
if qsave
    out_file = fullfile(pwd, 'figures', 'single_spin_flip',...
        strcat(fname_out,'_hamming_dist_traj')); % filename
...
h4 = figure(4);
bins = 0:0.05:1;
bins = 1:ceil(max(hamming));
histogram(hamming, bins, 'Normalization', 'pdf');
%xlim([0 max(hamming)+1]);
%xlim([0 1]);
xlabel('$$d_H(X_{base}, X_{perturb})$$', 'Interpreter', 'latex');
ylabel('Probability');
set(gca, 'FontSize', 24);
set(gcf, 'Units', 'Inches', 'Position', [1 1 10 8]);

% save equilibrium hamming dist
qsave = 0;
if qsave
    out_file = fullfile(pwd, 'figures', 'single_spin_flip',... 'no_noise', ...
        strcat(fname_out,'_eq_hamming_dist_hist')); % filename
