...
prec = 8; 
n_smpl = 100;
noise_all = 10.^[-0.6 -0.3]; %[-1:0.1:0.5];
...
save_fig_folder = 'H:\My Documents\Multicellular automaton\figures\finite_Hill\pin_pout\in_out_binary_v2\vs_noise';
%%
for run=1:numel(noise_all)
    close all;
    ...
    save_folder_data = 'H:\My Documents\Multicellular automaton\figures\finite_Hill\pin_pout\data_binary_v2\vs_noise';
    ...
    fname_str = strrep(sprintf('pin_pout_N%d_Con%d_K%d_a0_%.2f_hill%.2f_noise%.2f_prec%d_%s_nsmpl%d_run1',...
        gridsize^2, Con, K, a0, hill, noise, prec, sampmethod, n_smpl), '.', 'p');
    save_folder_data = fullfile('H:\My Documents\Multicellular automaton\figures\finite_Hill\pin_pout',...
        data_subfolder, 'vs_noise');
    %file_out = fullfile(pwd, 'figures', 'pin_pout', data_subfolder, strcat(fname_str,'.mat'));
    file_in = fullfile(save_folder_data, fname_str);
    load(file_in);
    %}
    ...
    if save_fig > 0
        ...
        out_file = fullfile(save_fig_folder, strcat(fname_str, '_fin_fout_v2') );
    ...
    %    N, K, Con, a0, Rcell),'FontSize', 18)
    xlabel('f_{in}')
    ylabel('Average I in equilibrium')
    ylim([-0.2 0.8]);
