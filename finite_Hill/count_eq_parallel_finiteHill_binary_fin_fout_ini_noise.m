function [peqcount, dHcount, t_av, I_av, dH_mean, dH_std] =...
    count_eq_parallel_finiteHill_binary_fin_fout_ini_noise(dist, Con, K, a0, Rcell,...
...
for k = 0 : N
    ...
    if k == 0 || k==N %special case all cells OFF
        ...
        cells_binary = cells>fp(2); % map to binary cells
        cells_base_out = cells_binary; % store base cells 
        kout = sum(cells_binary); %round(sum(cells));
        ...
        for flip_idx = 1:n_smpl*noise_smpl
            ...
            cells_binary = cells>fp(2);
            dH_out = sum(abs(cells_binary-cells_base_out));
        ...
        parfor i = 1:n_smpl
            ...
            for flip_idx = 1:noise_smpl
                ...
                % compute Hamming distance
                cells_binary = cells>fp(2);
                dH_aux(i, flip_idx) = sum(abs(cells_binary-cells_base_out));
            end
        end
