...
% Compare binary and continuous system. Did not give the same/similar
% dynamics.
close all
clear all
warning off
...
tmax = 200;
sigma_min = 0.1; % start binary simulation when sigma < sigma_min
...
while changed && t<tmax && (std(cells_out) > sigma_min)
    t = t+1;
...
%% run binary simulation
cells = cells_hist{1} > fp(2);
Con_2 = fp(3)*Con;
Coff_2 = fp(1);
K_2 = K; %K*(1/fp(2) - 1)^(1/hill);
...
[cells_out, changed, ~] = update_cells_Coff(cells, dist, Coff_2, Con_2, K_2, a0, Rcell);
while changed
    %pause(1);
    %k = waitforbuttonpress;
    t2 = t2+1;
