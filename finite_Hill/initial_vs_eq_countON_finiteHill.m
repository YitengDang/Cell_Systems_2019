...
fname = strrep(sprintf('pin_pout_N%d_Con%d_K%d_a0_%.2f_hill%.2f_prec%d_%s', ...
    gridsize^2, Con, round(K), a0, hill, prec, sampmethod), '.', 'p');
...
elseif strcmp(sampmethod, 'in_out_binary')
    [count, t_av, I_av] = count_eq_parallel_finiteHill_in_out_binary(...
        dist, Con, K, Rcell, a0, hill, prec, fp); %NB need fixed points
