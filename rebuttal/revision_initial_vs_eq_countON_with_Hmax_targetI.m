...
I0 = 0; % target I
[count, t_av] = count_eq_parallel_I(dist, Con, K, a0, Rcell, I0);
p = (0:N)./N;
N1 = round(0.2*N);
N2 = round(0.8*N);
px = (N1:N2)/N;
...
idx = find(max(h(p, 0)) == h(p,0)); % find index corresponding to p*
ps = p(idx); %p*
plot([ps ps], [0 1], 'g--', 'LineWidth', 2);

% Plot only H(p,0)
yyaxis right
plot(p, h(p,0), 'b-', 'LineWidth', 1.5);
ylabel('h(p, 0)');
...
hold on
Iv = -0.05:0.05:1;
[pmesh,Imesh] = meshgrid(p, Iv);
contourf(p, Iv, h(pmesh, Imesh),'LineStyle', 'none')
colormap('summer')
c = colorbar;
