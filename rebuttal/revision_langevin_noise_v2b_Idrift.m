...
Klist = [20 10 10]; %[6 20 12]; 
Conlist = [12 20 10]; %[15 15 15];
%a0list = [0.2 0.5 0.8 1.0 1.2 1.5];

% Comment out appropriate variables below (and above)
...
nruns = round(N/2); % number of simulations
...
path = fullfile(pwd, 'rebuttal', 'trajectories', 'Inoise_effect_a0', 'N18_a0_1_test');
...
delh_delI = @(p,I) 2*fN*(Con - 1)*p.*(p - 1)*phi;
...
for i=1:nruns
    for j=1:maxsteps 
        pt = p_stoch(i, j);
        It = I_stoch(i, j);
        [p_new, I_new, Peq, p_lim] = revision_update_Langevin(pt, It, N, Con, K, fN, gN, delta, noise, alpha, 1/phi);
        Peq_all(i,j) = Peq;
...
% Save trajectories
qsave = 0;
if qsave && masterqsave
    fname_str = strrep(sprintf('N%d_pini_range_nruns%d_a0_%.1f_K_%d_Con_%d_delta_%.3f_noise_%d', ...
        N, nruns, a0, K, Con, delta, noise), '.', 'p');
    %fname_str = strrep(sprintf('N%d_pini_range_nruns%d_a0_%.1f_K_%d_Con_%d_delta_%.3f_noise_%d_Idrift_fmax%.1f_Ns_%d', ...
    %N, nruns, a0, K, Con, delta, noise, fmax, Ns), '.', 'p');
