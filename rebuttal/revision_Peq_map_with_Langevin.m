...
%path = fullfile(pwd, 'rebuttal', 'trajectories', 'Inoise_effect_a0');
...
if qsave
    ...
    %    N, p_ini, a0, K, Con), '.', 'p');
    %fname_str = strrep(sprintf('N%d_pini_range_nruns%d_a0_%.1f_K_%d_Con_%d_zero_param_Langevin',...
    %    N, nruns, a0, K, Con), '.', 'p');
    fname_str = strrep(sprintf('N%d_a0_%.1f_K_%d_Con_%d_simplified', ...
        N, a0, K, Con), '.', 'p');
    %fname_str = strrep(sprintf('N%d_pini_range_nruns%d_a0_%.1f_K_%d_Con_%d_delta_1divN_noise_%d_phi_%.2f', ...
    %    N, nruns, a0, K, Con, sigma, phi), '.', 'p');
    fname = fullfile(pwd, 'rebuttal', 'Peq_map', strcat('Peq_vf_traj_', fname_str));
