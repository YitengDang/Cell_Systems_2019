...
%--------------------------------------------------------------------------
% v4: new approach to calculate delta, sigma from previous equation of
% motion (delta p, delta I) equation by Eduardo
%--------------------------------------------------------------------------
clear all
close all
clc
...
a0 = 0.9;
Rcell = 0.2*a0;
Klist = [10];  %(a0=1.5) [3 6 10 15 17 20]; % (a0=0.5) [10 10 14 19 16 18]; %[3]; 
Conlist = [15]; %(a0=1.5) [24 21 21 20 14 14]; % (a0=0.5) [5 21 16 14 8 6]; %[24];
%Klist = [6 20 12]; 
%Conlist = [15 15 15];
...
foldername = strrep(sprintf('N%d_a0_%.1f_K%d_Con%d', N, a0, K, Con), '.', 'p');
path = fullfile(pwd, 'rebuttal', 'trajectories', 'Fig_4_new_alt', foldername);
...
pnoise(:,1) = 0;
Inoise(:,1) = 0; %normrnd(0, 0.02, [nruns, 1]); % take larger sigma for initial spread
...
for i=1:nruns
    for j=1:maxsteps 
        pt = p_stoch(i, j);
        It = I_stoch(i, j);
        [p_new, I_new, Peq, p_lim] = revision_update_Langevin(pt, It, N, Con, K, fN, gN, delta, sigma, alpha);
        Peq_all(i,j) = Peq;
...
qsave = 1;
if qsave
    fname_str = strrep(sprintf('N%d_pini_range_nruns%d_a0_%.1f_K_%d_Con_%d_delta_%.3f_noise_%.3f', ...
        N, nruns, a0, K, Con, delta, sigma), '.', 'p');
    %fname = fullfile(pwd, 'rebuttal', 'h_vector_field', 'Langevin_v4', fname_str);
    fname = fullfile(pwd, 'data', 'trajectories', fname_str);
    save(fname);
end
