...
% 2D Ising square lattice with periodic boundary conditions
% H = sum_{<i,j>} sigma_i sigma_j (so interaction parameter = 1)
% Maximum value of H: 2*L^2
%--------------------------------------------------------
clear all;
close all;
clc;
...
while ~flat
    ...
    for i=1:10000 % Do 10000 MC steps before checking
        ...
        E = Wang_Landau_binning(h, edges); % energy bin of original system
        Ep = Wang_Landau_binning(hp, edges); % energy bin of flipped system
        %disp(h); disp(hp);
        %disp(g(E)); disp(g(Ep));
...
% Save figure
filename = sprintf('Wang_Landau_L%d_f_exp1_ffinal_exp10tomin8_pflat_%d_normalised', L, round(10*p_flat));
if save
    savefig(h, filename);
end
%%
