...
Mcomm = [1 0; 1 0]; % type i reacts to type j iff M_ij=1
Mcomm = logical(Mcomm);
...
if qsave 
    ...
    path2 = 'L:\BN\HY\Shared\Yiteng\Multicellularity\data\time_evolution_twocelltypes';
    %path ='K:\bn\hy\Shared\Yiteng\Multicellularity\data_twocelltypes\time_evolution';
    fname = fullfile(path2, fname_str);
...
fprintf('max I: = %.2f \n', max(max(max(max(squeeze(I_final_avg(:, :, :, :)))))) );
...
for k1=1:numel(K_loop)
    for k2=1:numel(K_loop)
        ...
        qsave = 1;
        if qsave
            fname_str = strrep(sprintf('I_out_avg_N%d_a0_%.1f_N1_%d_K1_%d_K2_%d_nruns_%d_%s_v2',...
                N, a0, N1,  K_loop(k1), K_loop(k2), nruns, subfolder), '.', 'p');
            path3 = fullfile('L:\BN\HY\Shared\Yiteng\Multicellularity\figures\time_evolution_twocelltypes', subfolder);
            %path = 'L:\BN\HY\Shared\Yiteng\Multicellularity\data\time_evolution_twocelltypes';
            %path ='K:\bn\hy\Shared\Yiteng\Multicellularity\data_twocelltypes\time_evolution';
            fname = fullfile(path3, fname_str);
            %fname = fullfile(pwd, 'data', 'time_evolution', fname_str);
...
for k1=1:numel(K_loop)
    for k2=1:numel(K_loop)
        h=figure(numel(K_loop)*k1+k2); % figure index = 4, 5, ...
        imagesc(Con_loop, Con_loop, (squeeze(Non1_final_avg(k1, k2, :, :))'+squeeze(Non2_final_avg(k1, k2, :, :))')/(N) );
        title(sprintf('$$K_1 = %d, K_2 = %d$$', K_loop(k1), K_loop(k2)));
        set(gca, 'YDir', 'normal', 'FontSize', 24);
        xlabel('$$C_{ON,1}$$')
        ylabel('$$C_{ON,2}$$')
        c=colorbar;
        ylabel(c, '$$\langle N_{ON} \rangle/N$$', 'Interpreter', 'latex')
        caxis([0 1]);
        qsave = 1;
        if qsave 
            fname_str = strrep(sprintf('pon_out_avg_N%d_a0_%.1f_N1_%d_K1_%d_K2_%d_nruns_%d_%s',...
                N, a0, N1,  K_loop(k1), K_loop(k2), nruns, subfolder), '.', 'p');
            path3 = fullfile('L:\BN\HY\Shared\Yiteng\Multicellularity\figures\time_evolution_twocelltypes', subfolder);
            %path = 'L:\BN\HY\Shared\Yiteng\Multicellularity\data\time_evolution_twocelltypes';
            %path ='K:\bn\hy\Shared\Yiteng\Multicellularity\data_twocelltypes\time_evolution';
