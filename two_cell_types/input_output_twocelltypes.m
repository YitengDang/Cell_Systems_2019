...
% output
fname_str = strrep(sprintf('input-output_N1_%d_N2_%d_a0_%.1f_K1_%d_K2_%d_Con1_%d_Con2_%d_nruns_%d_%s',...
        N1, N2, a0, K(1), K(2), Con(1), Con(2), nruns, Mcomm_str), '.', 'p');
savepath = fullfile(pwd, 'data', 'input-output_maps', Mcomm_str);
savepath_fig = fullfile(pwd, 'figures', 'input-output_maps', Mcomm_str);
...
%% Run trajectories
%
[p1_out, p2_out, theta11_out, theta12_out, theta22_out, I_out, t_final_out, h_out]...
    = input_output_twocelltypes_run(p1, p2, N1, N2, dist, a0, Con, K, Rcell,...
    Mcomm, tmax, nruns);
%}
