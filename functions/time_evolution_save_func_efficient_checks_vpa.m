function [cells_hist, period, t_onset] = time_evolution_save_func_efficient_checks_vpa(...
    N, a0, Rcell, lambda, hill, noise, M_int, K, Con, Coff,...
    distances, positions, sim_ID, mcsteps, InitiateI, p0, I0, cells_ini, tmax,...
    save_folder, fname_str_template, display_fig, vpa_digits)
    ...
    if save_folder % Check whether to save the result
        ...
        fname_str = strrep(sprintf('%s_t_out_%d_period_%d_vpa_digits_%d', fname_str_template,...
            t_out, period, vpa_digits), '.', 'p');
