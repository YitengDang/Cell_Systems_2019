function [cells_hist, period, t_onset] = time_evolution_save_multiple_sims(...
    N, a0, Rcell, lambda, hill, noise, M_int, K, Con, Coff,...
    distances, positions, sim_ID, mcsteps, InitiateI, p0, I0, cells_ini, tmax,...
    save_folder, fname_str_template, display_fig, num_sims)
    % Similar to time_evolution_save_func_efficient_checks, but saves
    % multiple simulations (n_sims) into one file
    
    % Also similar to time_evolution_save_func, but
    ...
    cells_hist_all = cell( num_sims, 1 );
    period_all = zeros( num_sims, 1 );
    t_out_all = zeros( num_sims, 1 );
    ...
    for i_sim = 1:num_sims
        ...
        if display_fig
            ...
            [h_cells, ~] = reset_cell_figure(hin, positions, rcell);
            disp_mol = 12;
            showI = 0; 
            t=0;
            %disp(size(cells));
        ...
        % store output
        cells_hist_all{i_sim} = cells_hist;
        period_all(i_sim) = period;
        t_out_all(i_sim) = t_out;
    end
    %----------------------------------------------------------------------
    % Save result
    I_ini_str = '';
    ...
    %    t_out, period), '.', 'p');
    fname_str = strrep(sprintf('%s_num_sims_%d', fname_str_template,...
        num_sims), '.', 'p');
    ...
    % save
    save(fname, 'save_consts_struct', 'cells_hist_all', 'period_all',...
        't_out_all', 'positions', 'distances');
