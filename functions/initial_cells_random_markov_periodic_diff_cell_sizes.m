function [pos, dist, fN0, rejections] =...
    initial_cells_random_markov_periodic_diff_cell_sizes(n, mcsteps, rcell_all, nodisplay)
...
for i=1:N
    ...
    fN(i) = sum(sinh(R_all)*sum(exp(R_all-r)./r)); % calculate signaling strength
end
fN0 = mean(fN);
if ~nodisplay
...
delta_all = (delx - 2*R_all)./4; % step size
step = 0;
rejections = 0;
while step < mcsteps
    %disp(step)
    ...
    x_new = x; 
    y_new = y;
    x_new(j) = mod(x_new(j) + delta_all(j)*rand(), Lx);
    y_new(j) = mod(y_new(j) + delta_all(j)*rand(), Ly);
