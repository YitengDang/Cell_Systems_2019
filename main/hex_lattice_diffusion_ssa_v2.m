...
% v2: multiple simulations, average over all
%%
clear variables
close all
rng('shuffle');
...
L = 11; %gridsize
d = 10^0; % diffusion rate
k = 10^2; % production rate
gamma = 10^(-0); % degradation rate

nsteps = 10^2; % simulation steps
nsim = 10^4; % number of simulations to average over

% initialisation
X0 = 0;
allX = X0*ones(nsim, L, L); % initial background concentration on lattice
cell = [1 1]; % secreting cell
allX(:, 1,1) = X0; % start with higher concentration on cell
t = []; % times
...
%% take average and display concentration on lattice
Xmean = squeeze(mean(allX, 1));
...
fname = strrep(sprintf('L%d_k_10e%.1f_gamma_10e%.1f_d_10e%.1f_X0_%d_nsim%d_nsteps%d',...
    L, log(k)/log(10), log(gamma)/log(10), log(d)/log(10), X0, nsim, nsteps), '.', 'p');
