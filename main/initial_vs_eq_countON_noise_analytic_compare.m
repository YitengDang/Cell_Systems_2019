...
% data from the Monte Carlo simulation is automatically.
clear variables
close all
warning off
...
[fname, path, ~] = uigetfile(fullfile(data_path,'pin_pout_noise'));
...
if save_fig > 0
    out_file = fullfile(pwd, 'figures', 'pin_pout_noise', strcat(name,'_map_sim'));
...
% Organize and save pdf
if save_fig > 0
    out_file = fullfile(pwd, 'figures', 'pin_pout_noise', strcat(name,'_time'));
...
% Organize and save
if save_fig > 0
    out_file = fullfile(pwd, 'figures', 'pin_pout_noise', strcat(name,'_map_analytic'));
...
if exist('I_av', 'var')
    ...
    % Organize and save
    if save_fig > 0
        out_file = fullfile(pwd, 'figures', 'pin_pout_noise', strcat(name,'_I'));
    ...
    % Organize and save
    if save_fig > 0
        out_file = fullfile(pwd, 'figures', 'pin_pout_noise', strcat(name,'_Ianalytic'));
