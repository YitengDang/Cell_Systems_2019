...
% V1: used to fix together two different sources of code
clear variables
close all
...
p1 = floor(0.1*N);
dp = round(0.02*N);
p = (p1:dp:N-p1)/N; % initial p
%p = (30:35)/N;
dI = 0.01; % accuracy of generated I
I_all = -0.2:0.05:1.0; % initial I
ntrials = 100; % trials per initial p,I point
times = zeros(numel(I_all), numel(p), ntrials);
I_out = zeros(numel(I_all), numel(p), ntrials);
...
cell_type = zeros(N, 1);
%nI_accept = zeros(numel(I_all), numel(p));
for k=1:numel(I_all_new)  
    I_min = I_all_new(k);
    I_max = I_min+dI;
    for i=1:numel(p)
        iniON = round(p(i)*N);
        ...
        parfor j=1:ntrials
            ...
            %[~, I_out(k,i,j), times(k,i,j), test_aux(j)] = generate_I_new(cells_in, I_min, I_max, dist, a0);
            [~, ~, ~, test_aux(j)] = generate_I_new(cells_in, I_min, I_max, dist, a0);
            %h2=figure();
            %update_cell_figure(h2, pos, 1, cells_out, cell_type, 0);
        end
        nI_accept_all(idx_start+k,i) = sum(test_aux);
    end
end
fracI_accept = nI_accept_all/ntrials;
%}
...
qsave = 0;
if qsave
   fname_str = strrep(sprintf('generate_I_N%d_a0_%.1f_I_all_Non_%d_%d_fracI_accept_approx_new',...
       N, a0, round(p(1)*N), round(p(end)*N)), '.', 'p');
...
%imagesc(p, I_all, fracI_accept);
%imagesc(p, I_all, nI_accept_all);
imagesc(p, I_all, data);
...
qsave = 0;
if qsave
   fname_str = strrep(sprintf('generate_I_times_N%d_a0_%.1f_I_%.3f_%.3f_fracI_accept_approx_new',...
...
%% Find limits of I for each value of p
% Define range of I as the range for which > 1/2 of simulations were
% accepted
I_range = zeros(numel(I_all), 2); %Only look at data satisfying this criterion
for i=1:size(data,2)
    idx = find(data(:,i) > 0);
    fprintf('p = %.2f | I = %.2f - %.2f \n', p(i), I_all(idx(1)), I_all(idx(end)) );
    I_range(i, 1) =  I_all(idx(1));
    I_range(i, 2) =  I_all(idx(end));
end

% Save limits for later use
qsave = 1;
if qsave
   fname_str = strrep(sprintf('generate_I_N%d_a0_%.1f_I_all_Non_%d_%d_Irange',...
       N, a0, round(p(1)*N), round(p(end)*N)), '.', 'p');
   fname = fullfile(pwd, 'figures', 'generate_I', 'data', fname_str);
   save(fname, 'p', 'I_range');
end
%% Mean generated I
%{
