...
for K = K_all
        cond1 = 1+fN>K; %all ON
        cond2 = 1+Con*fN<K; %OFF cannot turn ON
        cond3 = Con+fN>K; %ON cannot turn OFF
        cond4 = (1+fN)*Con < K; % all OFF
        ...
        fname_str = strrep(sprintf('vs_pin_N%d_Con_%d_K_%d_a0_%.2f_flip_%d', ...
            N, Con, K, a0, flip), '.', 'p');
        if final
            folder = fullfile(pwd, 'figures', 'spin_flip_final', 'N225_a0_1p50_K_1to30_Con_1to30');
        else
            folder = fullfile(pwd, 'figures', 'spin_flip_ini', 'N225_a0_1p50_K_1to30_Con_1to30');
        end
        % skip parameters that have already been examined
        if exist(fullfile(folder, 'data', strcat(fname_str, '-v1.mat')), 'file')==2
            continue
        end
        fprintf('Con=%d, K=%d \n', Con, K);    
        %% calculate the map
        ...
        if save_fig > 0
            ...
            v=1;
            fname = fullfile(folder,...
                strcat(fname_str,'-v', int2str(v), '_mean_std_lattice_flip'));
            while exist(strcat(fname, '.pdf'), 'file') == 2
                v=v+1;
                fname = fullfile(folder,...
                    strcat(fname_str,'-v', int2str(v), '_mean_std_lattice_flip'));
