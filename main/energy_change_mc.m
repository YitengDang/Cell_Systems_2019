...
straux = '(\d+)';
fpattern = sprintf('N%d_n%d_neq_%s_a0%d_K_%d_Son_%d_t_%s_montecarlo-v%s', ...
    N, iniON, straux, 10*a0, K, Son, straux, straux);
...
% save the histogram
name = sprintf('N%d_n%d_a0%d_K_%d_Son_%d_exact', ...
    N, iniON, 10*a0, K, Son);
out_file = fullfile( ...
    pwd,'figures', 'energy_change','no_noise_mc', strcat(name,'_h_change')); %filename
save_figure_pdf(h1, 8, 6, out_file);


% save file variables
fname_str = sprintf('N%d_n%d_a0%d_K_%d_Son_%d', ...
    N, iniON, 10*a0, K, Son);
i = 1;
fname = fullfile(pwd, 'data', 'energy_change', 'no_noise_mc', ...
