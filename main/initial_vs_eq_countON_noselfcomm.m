...
fname = sprintf('pin_pout_test_Con_%d_K_%d_gz_%d_a0_%d', ...
    Son, round(K), gridsize, 10*a0);
...
% calculate the map
[count, t_av] = count_eq_parallel_noselfcomm(dist, Son, K, a0, Rcell);
p = (0:N)./N;
prob = transpose(count./repmat(sum(count,2),1,N+1));

% save mat file
save(fullfile(data_path,'pin_pout',strcat(fname,'.mat')))
