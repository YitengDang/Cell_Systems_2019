% Creates pin-pout map from (p,I) Monte Carlo simulation
clear variables
close all
...
I_av = zeros(N+1, 3);
if Ii == 0
    fname = sprintf('pin_poutN%d_Con_%d_K_%d_gz_%d_a0_%d_montecarlo.mat', ...
            N, Con, K, gridsize, 10*a0);
else
    fname = sprintf('pin_poutN%d_Con_%d_K_%d_gz_%d_a0_%d_I%d_montecarlo.mat', ...
...
for n = 0:N
    ...
    for i = 1:trials
        ...
        t = 0;
        while cont
            [theta_new, p_new, pe] = update_montecarlo(theta, p, N, Con, K, fN, gN, alpha);
            if rand < pe
                cont = false;
...
%save(fullfile(pwd,'dynamics','pin_pout',fname));