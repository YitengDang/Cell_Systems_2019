...
for i = 1:numel(names)
    ...
    if numel(tokens) > 0
        ...
        figure(h2)
        plot(0:t, mom, 'Color', 'r')
        mom_all(end+1:end+numel(mom)) = mom;
    end
end
...
out_file = fullfile(pwd,'figures', 'hamiltonian_map',...
    'noise_MC', strcat(name,'_h_path')); %filename
...
name = sprintf('N%d_n%d_a0%d_K_%d_Son_%d_noise_%d_mc', ...
    N, iniON, 10*a0, K, Son, 10*noise);
out_file = fullfile(pwd, 'figures', 'hamiltonian_map', 'noise_MC',...
