% Analyze in detail the dynamics of cells with noise in time for a saved data. It
% plots the histograms for ON and OFF cells to see how it is the
% distribution of the concentration due to neighbors
...
for i = 2:numel(cells_hist)
    ...
    sigmaon = sqrt((Son-1)^2*(gN)*p*(1-p)+noise^2);
    sigmaoff = sqrt((Son-1)^2*(gN)*p*(1-p)+noise^2);
    ...
    hold off
    drawnow;
    pause(1) % wait before next time step
end