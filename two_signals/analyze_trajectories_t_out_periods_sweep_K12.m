...
save_path_fig = fullfile('H:\My Documents\Multicellular automaton\latex\10_two_signals\analyze_trajectories', ...
    'analyze_trajectories_vs_K12_temp');
...
n_trav_wave = zeros(nK12, 1);
p_av_all = zeros(nK12, nruns, 2);
pij_av_all = zeros(nK12, nruns, 4);
I_av_all = zeros(nK12, nruns, 2);
            ...
            for i = 1:num_files
                ...
                if strcmp(ext, '.mat')
                    ...
                    if ~isempty(tokens)
                        ...
                        t_out(idxK12, count+1) = this_t_out;
                        periods(idxK12, count+1) = this_period;
                        count = count + 1;
                        %names{count} = name;
                        
                        trav_wave = travelling_wave_test(cells_hist, a0, this_period, this_t_out);
                        n_trav_wave(idxK12) = n_trav_wave(idxK12) + trav_wave;
                        
                        [p_av, pij_av, I_av] = final_pI_av(cells_hist, a0, this_period, this_t_out);
                        p_av_all(idxK12, count+1, :) = p_av;
                        pij_av_all(idxK12, count+1, :) = pij_av;
                        I_av_all(idxK12, count+1, :) = I_av;
                    end
                end
            end
...
fname_str = sprintf('Sweep_K12_%d_to_%d_N%d_%s', K12_all(1), K12_all(end), N, lattice_lbl);
fname = fullfile(folder, strcat(fname_str, '_data'));
save(fname, 'N', 'a0', 'K12_all', 'nruns', 't_out', 'periods',...
...
h = figure(1);
hold on
plot(K12_all, f_stationary, 'k--', 'MarkerSize', 10);
scatter(K12_all, f_stationary, 100, 'k', 'filled', 'MarkerFaceAlpha', 0.5);

plot(K12_all, f_trav_wave, 'b--', 'MarkerSize', 10);
scatter(K12_all, f_trav_wave, 100, 'b', 'filled', 'MarkerFaceAlpha', 0.5);

plot(K12_all, f_period4, 'r--', 'MarkerSize', 10);
scatter(K12_all, f_period4, 100, 'r', 'filled', 'MarkerFaceAlpha', 0.5);

plot(K12_all, f_period_other, 'g--', 'MarkerSize', 10);
scatter(K12_all, f_period_other, 100, 'g', 'filled', 'MarkerFaceAlpha', 0.5);

% settings
xlim([K12_all(1) K12_all(end)])
xlabel('$$K_{12}$$');
ylabel('Fraction');
set(gca,'FontSize', 24);
set(h, 'Units', 'Inches', 'Position', [1 1 9 8]);
set(gca, 'XTick', K12_all(1):4:K12_all(end));

qsave = 0;
fname = fullfile(save_path_fig, strcat(fname_str, '_frac_all_types_vs_K12_',...
    num2str(nruns), 'runs'));
...
h = figure(1);
hold on
plot(K12_all, f_tmax, 'm--', 'MarkerSize', 10);
scatter(K12_all, f_tmax, 100, 'm', 'filled', 'MarkerFaceAlpha', 0.5);
...
%
yyaxis left
plot(K12_all, f_stationary, 'b--', 'MarkerSize', 10);
scatter(K12_all, f_stationary, 100, 'b', 'filled');
...
qsave = 0;
if qsave
    fname = fullfile(save_path_fig, strcat(fname_str, '_frac_nonperiodic_period4_vs_K12_',...
        num2str(nruns), 'runs'));
...
yyaxis right
plot(K12_all, n_trav_wave/nruns, 'k--', 'MarkerSize', 10);
scatter(K12_all, n_trav_wave/nruns, 100, 'k', 'filled', 'MarkerFaceAlpha', 0.5);
ylabel('Fraction travelling waves');
ylim([0 1.05]);
%}
% Plot with average I_final
%yyaxis left
plot(K12_all, I_av(:,1), 'b--'); %, 'MarkerSize', 10, 'MarkerFaceColor', 'b');
plot(K12_all, I_av(:,2), 'r--'); %, 'MarkerSize', 10, 'MarkerFaceColor', 'r');
p1=scatter(K12_all, I_av(:,1),  100, 'b', 'filled', 'MarkerFaceAlpha', 0.5);
p2=scatter(K12_all, I_av(:,2),  100, 'r', 'filled', 'MarkerFaceAlpha', 0.5);
...
qsave = 1;
if qsave
    fname = fullfile(save_path_fig, strcat(fname_str, '_frac_trav_wave_and_Ifinal_vs_K12_',...
        num2str(nruns), 'runs'));
    save_figure(h, 9, 8, fname, '.pdf');
end
...
h = figure(31);
hold on
plot(K12_all, p_av(:,1), 'b--'); %, 'MarkerSize', 10, 'MarkerFaceColor', 'b');
plot(K12_all, p_av(:,2), 'r--'); %, 'MarkerSize', 10, 'MarkerFaceColor', 'r');
p1=scatter(K12_all, p_av(:,1), 100, 'b', 'filled', 'MarkerFaceAlpha', 0.5);
p2=scatter(K12_all, p_av(:,2), 100, 'r', 'filled', 'MarkerFaceAlpha', 0.5);
legend([p1 p2], {'1', '2'});
...
h = figure(32);
hold on
plot(K12_all, pij_av(:,1), 'w--')%, 'MarkerSize', 10, 'MarkerFaceColor', 'b');
plot(K12_all, pij_av(:,2), 'y--')%, 'MarkerSize', 10, 'MarkerFaceColor', 'r');
plot(K12_all, pij_av(:,3), 'b--')%, 'MarkerSize', 10, 'MarkerFaceColor', 'r');
plot(K12_all, pij_av(:,4), 'k--')%, 'MarkerSize', 10, 'MarkerFaceColor', 'r');
p1=scatter(K12_all, pij_av(:,1), 100, 'w', 'filled', 'MarkerFaceAlpha', 1, 'MarkerEdgeColor', 'k');
p2=scatter(K12_all, pij_av(:,2), 100, 'y', 'filled', 'MarkerFaceAlpha', 0.7, 'MarkerEdgeColor', 'k');
p3=scatter(K12_all, pij_av(:,3), 100, 'b', 'filled', 'MarkerFaceAlpha', 0.7, 'MarkerEdgeColor', 'k');
p4=scatter(K12_all, pij_av(:,4), 100, 'k', 'filled', 'MarkerFaceAlpha', 0.6, 'MarkerEdgeColor', 'k');
...
if qsave
    ...
    fname = fullfile(save_path_fig, strcat(fname_str, '_pij_av_vs_K12_',...
        num2str(nruns), 'runs'));
...
scatter(x1, y1);
scatter(x2, y2, 'r');
plot([K12_all(1) K12_all(end)], [tmax tmax], 'r--');
...
qsave = 1;
if qsave
    fname = fullfile(save_path_fig, strcat(fname_str, '_K12_vs_t_out_mean_scatter_', ...
        num2str(nruns) ,'runs'));
    save_figure(h, 9, 8, fname, '.pdf');
end
...
p=boxplot(t_out', K12_all); %, 'PlotStyle', 'compact' );
...
ylim(10.^[0 5]);
set(gca, 'XTick', 1:2:numel(K12_all), 'xticklabel', K12_all(1):2:K12_all(end));
set(gca, 'YTick', 10.^(0:5));
...
x = K12_data(:); %+rand(size(K12_data(:)))*0.1; % randomize x positions
y = periods(:);
...
for i=1:numel(uniq_periods)
    plot_list(i) = plot([K12_all(1)-1 K12_all(end)+1], [uniq_periods(i) uniq_periods(i)], '--', 'LineWidth', 0.5);
end
for i=1:nK12
    K12 = K12_all(i);
    % get mean & std
...
for idxK12=1:nK12
    ...
    xlabel('period');
    ylabel('count');
    title(sprintf('K12=%d, %d simulations', K12, nruns));
    set(gca,'FontSize', 24);
    ...
    if qsave
        fname = fullfile(save_path, strcat('K12_',...
