...
n_pset = 10000; %10000; % number of parameter sets to do
...
parent_folder = 'N:\tnw\BN\HY\Shared\Yiteng\two_signals_batch_sim_2\batch_sim_all_topologies_OR_logic_run1';
sim_ID = 'two_signals_OR';
...
for k=1:81
    ...
    fname = fullfile(parent_folder, subfolder1, 'parameters_all.mat');
    if exist(fname, 'file')==2
        try 
            load(fname, 'x');
        catch
            if strcmp(ME.identifier, 'MATLAB:load:couldNotReadFile')
                error('Could not find parameters_all.mat file for network %d', k);
            end
        end
    else
    ...
    cells_ini = [];
    fname_str_template = sprintf('batch_sim_OR_logic_network_%d', count);
    display_fig = 0;
    %mcsteps = 0;
    ...
    for idx1=1:n_pset
        ...
        %pattern = 'all_topologies_simulate-v(\d+)';
        pattern = sprintf('batch_sim_OR_logic_network_%d_t_out_%s_period_%s-v%s',...
            k, '(\d+)', '(\d+|Inf)', '(\d+)');
        ...
        for count=1:sim_todo
            ...
            time_evolution_save_func_efficient_checks_OR_logic(...
                N, a0,...
                Rcell, lambda, hill, noise, M_int, thisK, thisCon, Coff,...
                dist, pos, sim_ID, mcsteps, InitiateI, p0, I0, cells_ini, tmax,...
