function [cells_hist, period, t_onset] = time_evolution_save_func_efficient_checks_run2(...
    ...
    while changed && period==Inf && t<t_ac
        ...
        %update_cell_figure_continuum(app, pos, distances, a0, cells, app.Time, cell_type, disp_mol, 0);
        [cellsOut, changed] = update_cells_two_signals_multiply_finite_Hill(cells, distances, M_int, a0,...
            Rcell, Con, Coff, K, lambda, hill, noise);
    end
    ...
    fname_str = strrep(sprintf('%s_N%d_hill_%.2f_K12_%d_t_out_%d_period_%s',...
                sim_ID, N, hill, K(1,2), t_out, num2str(period)), '.', 'p');
    ...
    save_vars_lbl = {'N', 'a0', 'K', 'Con', 'Coff', 'M_int', 'hill',...
        'noise', 'p_ini', 'I_ini', 'rcell', 'Rcell',  'lambda12', 'lambda', ...
        'sim_ID', 'I_ini_str', 'tmax', 'mcsteps'};
