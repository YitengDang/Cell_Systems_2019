...
period = periodicity_test(cells_hist, save_consts_struct);
idx = find(period~=0, 1);
if ~isempty(idx)
...
t1 = 1;
fig_pos = [1 1 8 6];
msg = plot_p_vs_t(cells_hist(t1:end), fig_pos);
disp(msg);
...
t1 = 1;
fig_pos = [1 1 8 6];
msg = plot_I_vs_t(cells_hist(t1:end), a0, dist, 1, fig_pos);
disp(msg);
...
if qsave
    h = figure(2);
    [~, fname_str] = fileparts(file);
    path_out = fullfile(fig_folder, strcat(fname_str, '_I_vs_t'));
    ext = '.pdf';
    save_figure(h, 10, 8, path_out, ext)
end