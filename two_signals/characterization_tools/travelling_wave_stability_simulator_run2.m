...
cells_load = cell(2,1);
cells_load{1} = xlsread(fname, 1);
try
    cells_load{2} = xlsread(fname, 2);
catch
    warning('No second sheet found');
end
...
unmodified_all = zeros(); % is the travelling wave unmodified (on population level)
for k=1:numel(K_22_all)
    for i=1:numel(K_12_all)
        for j=1:numel(K_21_all)
            ...
            % save results
            trav_wave_all(i,j,k) = trav_wave;
            unmodified_all(i,j,k) = all(n_out==n_in);
        end        
    end
end
close all
...
%% Load data
%{
load_folder = 'H:\My Documents\Multicellular automaton\figures\two_signals\trav_wave_stability';
fname_str_data = sprintf('%s_stability_sim_%s_K12_K21_K22_range', fname_str, fname_str_default);  
