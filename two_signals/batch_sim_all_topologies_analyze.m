...
for i=1:numel(network_sel)
    scatter( unique(period_all_net(idx_sort(i),:)), ...
        repmat(i, period_count(idx_sort(i)), 1), 35, 'bd', 'filled' );
end
set(gca, 'YTick', 1:numel(network_sel), 'YTickLabels', network_sel(idx_sort));
set(gca, 'XScale', 'log');
xlim([1 10^4]);
ylim([0 numel(network_sel)+1]);
plot([4 4], [0 numel(network_sel)+1], 'r--');
plot([2 2], [0 numel(network_sel)+1], 'g--');
set(gca, 'FontSize', 13);
ylabel('Topology number', 'FontSize', 20);
xlabel('Periods', 'FontSize', 20);

% Same topologies as those with high tmax!
qsave = 1;
fname_str = 'oscillations_all_scatter_subset3';
save_figure(h, 8, 8, fullfile(save_folder, fname_str),'.pdf', qsave);

%% Ubiquity of oscillations
