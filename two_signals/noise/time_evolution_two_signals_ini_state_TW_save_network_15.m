...
sim_count = 3;
max_num_params = 100; % max. # parameter sets to do
networks_all = [15 19 33 33 34 36]; 
network = 15; 
appendix = ''; % Note special rule for 33a, 33b
tmax = 10^2; % max. number of time steps 
...
num_params = min( size(Con_wave_sim, 1), max_num_params ); % number of parameter sets to simulate
...
%(!!!)  % Filename pattern (!!!)
        pattern = strrep(sprintf('%s_N%d_ini_state_TW_params_%d_noise_%.3f_t_out_%s_period_%s',...
        ...
        fprintf('N=%d, parameter set %d, sigma_D = %.2f sim to do: %d \n',...
...
for idx_loop=1:numel(noise_all) %numel(mcsteps_all)
    noise = noise_all(idx_loop);
    %sigma_D = sigma_D_all(idx_loop);
    %mcsteps = mcsteps_all(idx_loop);
    for trial=1:sim_count
        ...
        positions = {};
        distances = {};
        fname_str_template = strrep(sprintf('%s_N%d_ini_state_TW_params_%d_noise_%.3f',...
