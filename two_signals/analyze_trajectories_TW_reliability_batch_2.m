...
% batch 2: simulate trajectories with parameters that can propagate waves,
% and check whether they can still self-organize into waves
clear all
close all
...
num_params_all = [2534 0 0 0 0];
nruns_all = [100 0 0 0 0];

prev_param_count = 2000; % previous parameter count, can directly load data from there

% folder for saving figures
save_path_fig = 'H:\My Documents\Multicellular automaton\figures\two_signals\trav_wave_reliability_batch_2';
...
for network_idx = 1 %:numel(network_all)
    ...
    subfolder = sprintf('Network_%d_N%d', network, N);
    fname_str = sprintf('analyzed_data_%s_nruns_%d_nparams_%d_digits_5', subfolder, nruns, prev_param_count);
    load( fullfile(save_data_path, strcat(fname_str, '.mat') ),...
        'filecount', 'K_all', 'Con_all', 'tmax', 't_out_all', 'period_all', 't_onset_all',...
        'trav_wave_all', 'trav_wave_all_2', 'hom_end_state_all');

    zeros_array = zeros( num_params-prev_param_count, nruns );
    t_out_all = [t_out_all; zeros_array];
    period_all = [period_all; zeros_array];
    t_onset_all = [t_onset_all; zeros_array];
    trav_wave_all = [trav_wave_all; zeros_array];
    trav_wave_all_2 = [trav_wave_all_2; zeros_array];
    hom_end_state_all = [hom_end_state_all; zeros_array];
    Con_all = [Con_all; zeros(num_params-prev_param_count, 2)];
    K_all = [K_all; zeros(num_params-prev_param_count, 2, 2)];
    %% Load raw data files
    %
    %%% PART I %%%
    ...
    for ii = prev_param_count+1:num_params
        subfolder2 = sprintf('Param_%d', ii);
    ...
    % --- Manually set ---
    pattern = sprintf(...
        'Simulate_network_%d_params_%s_t_out_%s_period_%s-v%s',...
        network, '(\d+)', '(\d+)', '(\d+|Inf)', '(\d+)'); % '.' = anything
    % --------------------
    subfolder = sprintf('Network_%d_N%d', network, N);
    for ii=prev_param_count+1:num_params
        subfolder2 = sprintf('Param_%d', ii);
        ...
        fname_data = fullfile(folder, 'parameters.mat');
        load(fname_data, 'thisCon', 'thisK');
        Con_all(ii, :) = thisCon;
        K_all(ii, :, :) = thisK;
    ...
    %
    subfolder = sprintf('Network_%d_N%d', network, N);
    fname_str = sprintf('analyzed_data_%s_nruns_%d_nparams_%d_digits_5', subfolder, nruns, num_params);
...
%}
%% Plot overall reliability per network
%
h=figure;
hold on
box on
...
for network_idx=1 %2:5 %[1 2 4 5]
    ...
    qsave = 0;
    fname = fullfile(save_path_fig, strcat('analyzed_data_', subfolder,...
        sprintf('_nruns_%d_digits_%d', nruns, digits), '_classification_bar_sorted_TW_only'));
    save_figure(h, 12, 8, fname, '.pdf', qsave);
    ...
    set(gca, 'FontSize', 32);
    %}
    %hist_data = trav_wave_all_2_mean;
    hist_data = trav_wave_all_2_mean(trav_wave_all_2_mean>0);
    ...
    if qsave
        ...
        fname = fullfile(save_path_fig, strcat('analyzed_data_', subfolder,...
            sprintf('_nruns_%d_digits_%d', nruns, digits), '_frac_TW_all_histogram_v2_nonzero'));
        save_figure(h, 10, 8, fname, '.pdf', qsave);
    end
    %}
    %% Fraction of simulations with reliability>0
    count_reliability = sum(trav_wave_all_2_mean>0);
    
    h = figure;
    pie([count_reliability numel(trav_wave_all_2_mean)-count_reliability])
    legend('Reliability>0', 'Reliability=0');
    set(gca, 'FontSize', 20);
    
    qsave = 0;
    if qsave
        fname = fullfile(save_path_fig, strcat('analyzed_data_', subfolder,...
            sprintf('_nruns_%d_digits_%d', nruns, digits), '_pie_chart_frac_reliable'));
        save_figure(h, 10, 8, fname, '.pdf', qsave);
    end
    ...
    idx_nonzero = find(frac_all(:,1)>0);
    idx_zero = find(frac_all(:,1)==0);
    ...
    p_zero = scatter( K_all(idx_zero, 1, 1), Con_all(idx_zero, 1), sz, frac_all(idx_zero, 1), 'filled' ); % 1<-1
    p_nonzero = scatter( K_all(idx_nonzero, 1, 1), Con_all(idx_nonzero, 1), sz, frac_all(idx_nonzero, 1), 'filled' ); % 1<-1
    xlabel('K^{(11)}');
    ylabel('C_{ON}^{(1)}');
    c11=colorbar;
    ...
    % plot >0 and =0 separately
    p_zero = scatter( K_all(idx_zero, 1, 2), Con_all(idx_zero, 2), sz, frac_all(idx_zero, 1), 'filled' ); % 1<-1
    scatter( K_all(idx_nonzero, 1, 2), Con_all(idx_nonzero, 2), sz, frac_all(idx_nonzero, 1), 'filled' ); % 1<-1
    xlabel('K^{(12)}');
    ylabel('C_{ON}^{(2)}');
    c12=colorbar;
    ...
    % plot >0 and =0 separately
    p_zero=scatter( K_all(idx_zero, 2, 1), Con_all(idx_zero, 1), sz, frac_all(idx_zero, 1), 'filled' ); % 1<-1
    scatter( K_all(idx_nonzero, 2, 1), Con_all(idx_nonzero, 1), sz, frac_all(idx_nonzero, 1), 'filled' ); % 1<-1
    xlabel('K^{(21)}');
    ylabel('C_{ON}^{(1)}');
    ...
    if network_idx>=3
        ...
        scatter( K_all(idx_zero, 2, 2), Con_all(idx_zero, 2), sz, frac_all(idx_zero, 1), 'filled' ); % 1<-1
        scatter( K_all(idx_nonzero, 2, 2), Con_all(idx_nonzero, 2), sz, frac_all(idx_nonzero, 1), 'filled' ); % 1<-1
        xlabel('K^{((22))}');
        ylabel('C_{ON}^{(2)}');
        c22=colorbar;
    ...
    qsave = 1;
    fname_str = sprintf('Psets_network_%d_nruns_%d_by_reliability_K_Con_maps_v4_jet', network, nruns);
    fname = fullfile(save_path_fig, fname_str);
    save_figure(h, 16, 12, fname, '.pdf', qsave);
    %% Spider charts
    ...
    c_idx = floor(TW_frac./(1/n_colors))+1;
    c_idx(c_idx==n_colors+1) = n_colors;
