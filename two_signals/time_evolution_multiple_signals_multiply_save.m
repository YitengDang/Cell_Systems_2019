...
Coff = [1 1];
M_int = [1 -1; 1 0];
K = [8 25; 35 0]; % K(i,j): sensitivity of type i to type j molecules
...
for i1 = 1:size(p1, 1)
    for i2 = 1:size(p1, 2)
        ...
        for run=1:nruns
            ...
            % filename 
            %SimID = {'one_signal', 'two_signal_mult', 'two_signal_add'};
            ...
            i=1;
            fname = fullfile(folder, strcat(filename, '-v', num2str(i), label, ext));
            ...
            save(fname, 'save_consts_struct', 'cells_hist', 't_out', 'changed');
