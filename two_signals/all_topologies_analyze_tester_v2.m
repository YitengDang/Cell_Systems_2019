%function [A, ss, cycles] = all_topologies_analyze(P)
    ...
    M_int = [1 0; 1 1];
    single_cell = 1;
    %phase = [2 2; 2 2]; % 1: inactive (Con < K), 2: active (Con > K), 0: no connection
    phase = [3 3; 3 0];
    draw_diagram = 1;
    
    [A, ss, cycles] = all_topologies_analyze(single_cell, phase, M_int, draw_diagram);
    ...
    for i=1:numel(cycles)
        %disp(cycles{i});
        disp(states(cycles{i}));
    end
    
    %---------Modification of plot_state_diagram.m---------
