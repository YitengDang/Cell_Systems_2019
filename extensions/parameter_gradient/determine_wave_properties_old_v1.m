...
% This function also determines whether waves are bent ('met een knik')
...
% and the number of bands may be incorrect, however in the examples so far
...
% specific waveband colour. Useful when calculating band_vec at t + 1.
...
% wave_state -> see explanation above
...
if strcmp(orientation,'Horizontal')
    col1 = M_columns(1,:);
    bands = intersect(col1,wave_state_cell_idx);
    band_vec = zeros(length(col1), 1);
    for i = 1:length(col1)
        band_vec(i) = ismember(col1(i), bands);
    end
...
elseif strcmp(orientation,'Vertical')
    row1 = M_rows(1,:);
    bands = intersect(row1,wave_state_cell_idx);
    band_vec = zeros(length(row1),1);
    for i = 1:length(row1)
        band_vec(i) = ismember(row1(i),bands);
    end
...
elseif strcmp(orientation,'Diagonal')
    bands_in_wave = NaN;
    number_of_waves = NaN;
    x_wrap = length(intersect(M_rows(1,:),wave_state_cell_idx));
    y_wrap = length(intersect(M_columns(1,:),wave_state_cell_idx));
