%% Analyzse saved trajectories across a range of parameters
clear all
close all
...
save_path_fig = 'N:\tnw\BN\HY\Shared\Yiteng\two_signals\randomized lattice\TW_formation_network_15';
...
for p_set = 1:num_params
    ...
    % mcsteps
    pattern = sprintf(...
        'two_signal_mult_N%d_params_%s_sim_%s_mcsteps_%s_t_out_%s_period_%s-v%s',...
        N, '(\d+)', '(\d+)', '(\d+)',  '(\d+)', '(\d+|Inf)','(\d+)'); % '.' = anything
...
%
fname_str = sprintf('analyzed_data_%s_vs_mcsteps_num_params_%d_nruns_%d_digits_%d',...
    subfolder, num_params, nruns, digits);
save_data_path = 'N:\tnw\BN\HY\Shared\Yiteng\two_signals\randomized lattice\TW_formation_network_15';
...
fname_str = sprintf('analyzed_data_%s_vs_mcsteps_num_params_%d_nruns_%d_digits_%d',...
    subfolder, num_params, nruns, digits);
data_path = 'N:\tnw\BN\HY\Shared\Yiteng\two_signals\randomized lattice\TW_formation_network_15';
%save( fullfile(save_data_path, strcat(fname_str, '.mat')), 'noise_all',...
%    'filecount', 't_out_all', 'period_all', 't_onset_all', 'tmax',...
%    'num_params', 'nruns', 'digits', 'trav_wave_all', 'trav_wave_all_2');
load( fullfile(data_path, strcat(fname_str, '.mat')), 'mcsteps_all',...
    'filecount', 't_out_full', 'period_full', 't_onset_full', 'tmax',...
    'num_params', 'nruns', 'digits', 'trav_wave_full', 'trav_wave_full_2');
...
set(gca, 'XTick', 1:numel(mcsteps_all), 'XTickLabels', {'0', '1', '10', '100', '1000'} );
...
fname = fullfile(save_path_fig, strcat(subfolder,...
    sprintf('_%s_nruns_%d_digits_%d', label2, nruns, digits), '_classification_dynamics'));
...
xlabel('Lattice disorder amount');
%xlabel('Lattice disorder (Monte Carlo steps)');
%xlabel('Noise strength \alpha/K^{(ij)}');
ylabel('Fraction TW');
...
xticks = 10.^(-1:3);
% Tick labels
xtick_labels = sprintfc('10^{%d}', -1:4);
xtick_labels{1} = '0';
set(gca, 'FontSize', 32, 'XTick', xticks, 'XTickLabels', xtick_labels);

qsave = 1;
fname = fullfile(save_path_fig, strcat('TW_formation_', label2, '_', label1,...
...
qsave = 1;
fname = fullfile(save_path_fig, sprintf('TW_formation_%s_%s_nruns_%d_digits_%d',...
    label1, label2, nruns, digits, '_frac_TW_all_by_pset'));
save_figure(h, 12, 8, fname, '.pdf', qsave);
