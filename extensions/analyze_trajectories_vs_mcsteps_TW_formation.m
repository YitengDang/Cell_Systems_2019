...
%
pattern = sprintf(...
    'two_signal_mult_N%d_ini_state_rand_fixed_params_mcsteps_%s_t_out_%s_period_%s-v%s',...
    N, '(\d+)', '(\d+)', '(\d+|Inf)', '(\d+)'); % '.' = anything
%}
% noise
%{
pattern = sprintf(...
    'two_signal_mult_N%d_ini_state_rand_fixed_params_noise_%s_t_out_%s_period_%s-v%s',...
