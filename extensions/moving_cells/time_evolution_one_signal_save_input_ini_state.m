...
if hill<Inf
    ...
    if InitiateI
        ...
        dI = 0.1;
        nodisplay = 1;
        [cells_ini, test, I_ini, t_out] = generate_I_new(cells_ini, I0, I0+dI, dist, a0);
    end
end
...
for idx_loop=1:numel(sigma_D_all)
    ...
    % Filename pattern
    pattern = strrep(sprintf('%s_sigma_D_%.3f_t_out_%s',...
            sim_ID, sigma_D, '(\d+)'), '.', 'p');
...
for idx_loop=1:numel(sigma_D_all)
    ...
    for trial=1:sim_count
        ...
        %}
        display_fig = 0;
        [cells_hist, period, t_onset] = time_evolution_save_func_one_signal_moving_cells(...
