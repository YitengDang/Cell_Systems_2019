...
tmax = 2*10^3; % max. number of time steps 
...
for i=1:2
    ...
    if InitiateI && hill==Inf
        ...
        pause(0.01);
        dI = 0.1;
        [cells_temp, test, I_ini] = generate_I_new(cells_ini(:, i), I0(i), I0(i)+dI, distances, a0);
...
for idx_loop=1:numel(sigma_D_all)
    ...
    pattern = strrep(sprintf('%s_sigma_D_%.3f_t_out_%s-v%s',...
            sim_ID, sigma_D, '(\d+)', '(\d+)'), '.', 'p');
