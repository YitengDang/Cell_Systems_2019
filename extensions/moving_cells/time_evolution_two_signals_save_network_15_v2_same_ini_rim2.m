...
pset_idx = 20; % specific parameter set to start at
...
sim_to_do = zeros(num_params, numel(var_all));
...
for idx_param_loop=pset_idx:pset_idx+num_params-1
    ...
    for idx_loop=1:numel(var_all)
        sigma_D = sigma_D_all(idx_loop);
        %mcsteps = mcsteps_all(idx_loop);
        % noise = noise_all(idx_loop);       

%(!!!)  % Filename pattern (!!!)
        pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_sigma_D_%.3f_t_max_%d_period_%s',...
            sim_ID, N, idx_param_loop, '(\d+)', sigma_D, tmax, '(\d+|Inf)' ),...
            '.', 'p');
        ...
        fprintf('N=%d, parameter set %d, sigma_D = %.3f, sim to do: %d \n',...
            N, idx_param_loop, sigma_D, sim_count-filecount);
...
for idx_param_loop=pset_idx:pset_idx+num_params-1
    ...
    for trial=55:sim_count
        ...
        for idx_loop=1:numel(var_all)
            ...
            fprintf('Param. set %d, sigma_D %.3f, trial %d \n', idx_param_loop, sigma_D, trial);
            
            % ----------- simulation ------------------------------------
            display_fig = 0;
            % set to empty sets for non-lattice simulations
            positions = {};
            distances = {};
            fname_str_template = strrep(sprintf('%s_N%d_params_%d_sim_%d_sigma_D_%.3f',...
                sim_ID, N, idx_param_loop, trial, sigma_D), '.', 'p');
            %{
            ...
            [cells_hist, period, t_onset] = time_evolution_save_func_efficient_checks_moving_cells(...
                ...
                growth_rate, R_division, sim_ID, tmax, save_folder, fname_str_template, display_fig);
            %}
            %--------------------------------------------------------------------------
            %}
        end
