...
% motion of each cell
% Corrected the logistic growth function 
close all
clear all
% maxNumCompThreads(4);
% warning off
...
% growth parameters
sigma_rcell = 0.2; % initial spread in cell radii
rcell = 0.2;
Rcell = rcell*a0;
...
k_growth = 2; % max. fold-change of density
...
[rcell_all_new, delta_A_cells] = calc_cell_sizes_logistic_growth(rcell_all, c_growth, K_growth);
...
while t<tmax && cont_sim
    ...
    % continue if cells are still growing, or if cell states are changed
    fprintf('Summed change in area: %.4f \n', sum(abs(delta_A_cells)) );
    if sum(abs(delta_A_cells)) > 10^(-4) % if cells are still changing in size
        cont_sim = 1; % continue simulation
