...
A = {};
A{1} = '0';
A(2:5) = sprintfc('10^{%d}', -3:0);
%set(gca, 'XTick', -3:0, 'XTickLabel', sprintfc('10^{%d}', -3:0) );
set(gca, 'XTick', -4:0, 'XTickLabel', A);
set(gca, 'YTick', 0:0.2:1);
ylim([0 1]);
ylabel('Fraction of sustained TWs');
%xlabel('Cell motility \sigma_D');
xlabel('Cell motility');
set(gca, 'FontSize', 32);
set(h, 'Units', 'Inches', 'Position', [1 1 12 8]);
...
if qsave
    fname = fullfile(save_path_fig, strcat('analyzed_data_', subfolder,...
        sprintf('_nruns_%d_digits_%d', nruns, digits), '_frac_TW_preserved_vs_sigma_D_t1000_size_10_8_v2'));
