...
parent_folder = 'N:\tnw\BN\HY\Shared\Yiteng\two_signals\trav_wave_hill';
if remote
...
%    rcell, nodisplay);
[positions, distances] = initial_cells_random_markov_periodic_vpa(gz, mcsteps,...
    rcell, nodisplay, vpa_digits);
positions = double(positions);
distances = double(distances);
%% Calculate # required simulations
% Loop over Con, K values
sim_to_do = zeros(num_params, numel(hill_all));
...
for idx_param_loop=1:num_params
    ...
    for idx_loop=1:numel(hill_all)
        hill = hill_all(idx_loop);

%(!!!)  % Filename pattern (!!!)
        %{
        pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_hill_%d_t_out_%s_period_%s',...
            sim_ID, N, idx_param_loop, '(\d+)', hill, '(\d+)', '(\d+|Inf)' ),...
            '.', 'p');
        %}
        pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_hill_%d_vpa_digits_%d_t_out_%s_period_%s',...
            sim_ID, N, idx_param_loop, '(\d+)', hill, vpa_digits, '(\d+)', '(\d+|Inf)'),...
            '.', 'p');
        ...
        fprintf('N=%d, parameter set %d, hill = %d, sim to do: %d \n',...
            N, idx_param_loop, hill, sim_count-filecount);
...
for idx_param_loop=1:num_params
    ...
    for trial=1:sim_count
        ...
        for idx_loop=1:numel(hill_all)
            ...
            fprintf('Param. set %d, hill %d, trial %d \n', idx_param_loop, hill, trial);
            
            % ----------- simulation ------------------------------------
            display_fig = 0;
            %positions = {};
            %distances = {};
            fname_str_template = strrep(sprintf('%s_N%d_params_%d_sim_%d_hill_%d_vpa_digits_%d',...
                sim_ID, N, idx_param_loop, trial, hill, vpa_digits), '.', 'p');
            ...
            % VPA (not feasible for each time update step)
            %
            [cells_hist, period, t_onset] = time_evolution_save_func_efficient_checks_vpa(...
                N, a0, Rcell, lambda, hill, noise, M_int, K, Con, Coff,...
                distances, positions, sim_ID, mcsteps, InitiateI, p0, I0, cells_ini,...
                tmax, this_save_folder, fname_str_template, display_fig, vpa_digits);
            %}
            %{
