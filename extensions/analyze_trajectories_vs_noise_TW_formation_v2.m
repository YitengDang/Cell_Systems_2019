...
% v2: keep initial conditions constant
clear all
close all
...
N = 225;
tmax = 10000;
noise_all = [0 0.001 0.002 0.005 0.01 0.02 0.05 0.1 0.2 0.5 1];
%noise_all = [0 0.001 0.01 0.1 1];
network = 15;
...
for p_set = 1:num_params
    ...
    % noise
    pattern = sprintf(...
        'two_signal_mult_N%d_params_%s_sim_%s_noise_%s_t_out_%s_period_%s-v%s',...
        N, '(\d+)', '(\d+)', '(\d+p\d+)', '(\d+)', '(\d+|Inf)', '(\d+)'); % '.' = anything
    %pattern = sprintf(...
    ...
    for i=1:numel(names)
        ...
        if isempty(tokens) % file name doesn't match
            continue
        elseif str2double(tokens{1}{2}) > nruns % simulation count exceeds total # required sims
            continue
...
%xticklabels = {'0', '0.001', '0.002', '0.005', '0.01', '0.02', '0.05', '0.1', '0.2', '0.5', 1};
%set(gca, 'XTick', 1:numel(noise_all), 'XTickLabels', xticklabels);
...
set(gca, 'FontSize', 32);
xlabel('Noise strength \alpha/K^{(ij)}', 'FontSize', 32);
ylabel('Fraction of simulations', 'FontSize', 32);
box on
% classificiation I
...
fname = fullfile(save_path_fig, strcat('TW_formation_vs_noise_', label,...
    sprintf('_nruns_%d_digits_%d', nruns, digits), '_classification_dynamics_log'));
...
%x_data = noise_all;
hold on
plot( x_data, trav_wave_all_2_mean, 'ko--', 'LineWidth', 1.5 );
...
qsave = 1;
fname = fullfile(save_path_fig, strcat('TW_formation_vs_noise_', label,...
    sprintf('_nruns_%d_digits_%d', nruns, digits), '_frac_TW_all_mean_size_10_8_final'));
...
%% Analyze <t_out>
choice = 2; % choose between two options
if choice==1
    % (1) <t_out>
    t_out_mean = mean(mean(t_out_full, 3), 2);
    t_out_std = std(t_out_full(:,:), 0, 2);
    this_label = '_t_out_mean';
elseif choice==2
    % (2) Restrict to TW end states only
    t_out_full_temp = zeros(size(t_out_full));
    idx_TW = logical(trav_wave_full_2);
    t_out_full_temp(idx_TW) = t_out_full(idx_TW);
    t_out_mean = sum(sum(t_out_full_temp(:,:,:), 3),2)./sum(sum(idx_TW(:,:), 3), 2);
    this_label = '_t_out_mean_TW_only';
end
%
h = figure;
x_data = [10^(-4) var_all(2:end)];
...
elseif choice==2
    ylabel('\langle t_{out} | TW \rangle');
end
title(sprintf('t_{max}=10^{%d}', log10(tmax)));
...
if qsave
    fname = fullfile(save_path_fig, strcat('TW_formation_vs_noise_', label,...
        sprintf('_nruns_%d_digits_%d', nruns, digits), this_label));
    save_figure(h, 10, 8, fname, '.pdf', qsave);
end
...
figure;
hold on
for idx=1:numel(noise_all)
    data = squeeze(t_out_full(idx,idx_TW(idx,:,:)));
    histogram(data);
end
