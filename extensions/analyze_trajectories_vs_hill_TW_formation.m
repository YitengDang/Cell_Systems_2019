...
box on
% classificiation I
legend({'Travelling wave', 'Oscillatory', 'Static', 'Infinite dynamics'}, 'location', 'best');
...
qsave = 0;
fname = fullfile(save_path, strcat('TW_formation_vs_noise_', label,...
...
qsave = 1;
fname = fullfile(save_path, strcat('TW_formation_vs_mcsteps_', label,...
...
% Ticks
xticks = [fliplr(hill_all(2:end)) 15];
% Tick labels
xtick_labels = sprintfc('%d', fliplr(hill_all(1:end)));
%xtick_labels{end+1} = 'Inf';
set(gca, 'FontSize', 32, 'XTick', xticks, 'XTickLabels', xtick_labels);
...
if qsave
    fname = fullfile(save_path_fig, strcat('TW_formation_vs_mcsteps_', label,...
        sprintf('_nruns_%d_digits_%d', nruns, digits), '_t_out_mean'));
    save_figure(h2, 10, 8, fname, '.pdf', qsave);
end
