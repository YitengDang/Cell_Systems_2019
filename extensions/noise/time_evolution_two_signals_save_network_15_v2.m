...
noise_all = [0.002 0.005 0.02 0.05 0.2 0.5];
...
[positions, distances] = initial_cells_random_markov_periodic(gz, mcsteps,...
    rcell, nodisplay);
%% get all parameter sets from saved sims
%{
Con_wave_all = zeros(10, 2);
K_wave_all = zeros(10, 2, 2);

for idx_param_loop=1:10
    ...
    names_orig_sim = {};
    filecount = 0;
    pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_noise_%.3f_t_out_%s_period_%s',...
        sim_ID, N, idx_param_loop, '(\d+)', 0, '(\d+)', '(\d+)', '(\d+|Inf)' ),...
        '.', 'p');
    for i = 1:num_files
        filename = listing(i+2).name;
        [~, name, ext] = fileparts(filename);
        if strcmp(ext, '.mat')
            [tokens, match] = regexp(name, pattern, 'tokens', 'match');
            %disp(match);
            if ~isempty(match)
    ...
    % load file
    fname_load = fullfile(folder_orig_sim, names_orig_sim{1});
    temp = load(fname_load);
    Con_wave_all(idx_param_loop, :) = temp.save_consts_struct.Con;
    K_wave_all(idx_param_loop, :, :) = temp.save_consts_struct.K;    
end
%}
...
%(!!!)  % Filename pattern (!!!)
        pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_noise_%.3f_t_out_%s_period_%s',...
            sim_ID, N, idx_param_loop, '(\d+)', noise, '(\d+)', '(\d+|Inf)' ),...
            '.', 'p');
...
for idx_param_loop=1:num_params
    %% batch Con, K unreliable -> directly obtain from simulation files
    ...
    filecount = 0;
    pattern = strrep(sprintf('%s_N%d_params_%d_sim_%s_noise_%.3f_t_out_%s_period_%s',...
        sim_ID, N, idx_param_loop, '(\d+)', 0, '(\d+)', '(\d+|Inf)' ),...
        '.', 'p');
    ...
    for trial=1:sim_count
        % ============ load parameters and initial state ==================
        ...
        for idx_loop=1:numel(noise_all)
            ...
            %positions = {};
            %distances = {};
            fname_str_template = strrep(sprintf('%s_N%d_params_%d_sim_%d_noise_%.3f',...
                sim_ID, N, idx_param_loop, trial, noise), '.', 'p');
